...
- **绿色边界** = 当前允许继续承接客户使用、可以保护、可以继续在其上做小步优化的主线
- **红色边界** = 当前明确不应继续扩张、不应再误当主线、必须冻结或单独处理的问题区
...
flowchart TD
    ...
    A --> G1[绿色主线<br/>本地对话]
    A --> G2[绿色主线<br/>跨频道记忆 / 广播板]
    A --> G3[绿色主线<br/>大文件分片与按需读取]
    A --> G4[绿色主线<br/>定时任务主执行链]
    A --> G5[绿色主线<br/>本机微信扫码授权]
    A --> G6[绿色主线<br/>飞书 / Web / 微信已绑定后的正常对话]

    A --> R1[红色禁区<br/>服务器端微信官方扫码授权]
    A --> R2[红色禁区<br/>未冻结前的 IM 通知幻想能力]
    A --> R3[红色禁区<br/>skills / 插件 / MCP 无闸门外扩]
    A --> R4[红色禁区<br/>CoworkRunner 旧兼容链回流主线]
    A --> R5[红色禁区<br/>高 token 注入 / 默认全量加载]
    A --> R6[红色禁区<br/>未稳定前继续把 scheduler 扯复杂]

    classDef green fill:#e8fff0,stroke:#1f9d55,stroke-width:2px,color:#14532d;
    classDef red fill:#fff1f2,stroke:#dc2626,stroke-width:2px,color:#7f1d1d;
...
- 大文件分片、附件按需读取、工具选择加载
- scheduler 主执行链本身
- 本机 `127.0.0.1 / localhost` 的微信扫码授权
- 已绑定完成后的飞书 / 微信 / Web 正常消息流转

### 红色边界：当前必须明确切开的东西

- **服务器端微信官方扫码授权**
- **未稳定的 IM 通知前端体验**
- **未隔离的 skills / MCP / 插件外扩**
- **CoworkRunner 旧回退链重新回到主线**
- **默认全量技能注入、导致缓存命中差和 token 飙升**
- **在 scheduler 还没完全稳住前继续叠加复杂能力**

### 这张图的使用方式

- 绿区只做保护、补稳、轻量优化
- 红区先冻结、再拆分、再决定是否另起支线
- 任何新需求先判断它落在绿区还是红区，避免再把系统重新搅混
...
- **这块已推进**
- 后续还可以继续把附件读取进度也抬成同等级状态

### 2.13 `CoworkRunner` 主链退位判断已写清

已经补好的内容：

- 已单独写明：
  - 哪些地方已经脱离 `CoworkRunner`
  - 哪些地方还只是兼容壳
  - 哪些地方是 scheduler 真活口
  - 最小切除顺序是什么
...
- **范围已经挖出来**
- **scheduler 执行回退口已封死**
- **下一步观察并收口 stop 兜底 / 单例壳 / 适配缝**

相关文件：

- `docs/2026-03-31_043522_COWORKRUNNER_LEGACY_SCOPE_AND_CUT_ORDER.md`
...
- `docs/2026-03-31_045112_FEISHU_SOFT_TRANSITION_PLAN.md`
- `docs/2026-03-31_051305_FEISHU_IM_SENDER_PHASE1.md`
...
- `docs/2026-03-31_055109_FEISHU_PRIVATE_BINDING_PHASE1.md`
...
- `docs/2026-03-31_061921_PHASE1_WRAPUP_SCHEDULER_AND_FEISHU.md`

### 2.17 客户部署备份边界已写清

已经补好的内容：

- 明确客户数据库、`.env`、`.uclaw/` 属于不可直接覆盖的数据真相源
- 明确升级时只替换代码与静态资源
- 明确先备份再升级的规则
...
- **客户云端升级可以做**
- **但必须按“保数据、保配置、只换程序”执行**

相关文件：

- `docs/2026-03-31_063727_CUSTOMER_DEPLOY_BACKUP_BOUNDARY.md`
