...
- `docs/2026-04-08_TEAM_DEPENDENCY_DOOR_CARD.md`
  - 挂在门口的 `team` 依赖清单：先看它现在还借主服务端哪些接口、入口和资源层，再决定怎么拆、怎么部署、怎么给客户演示

- `docs/2026-04-08_ROLE_VISIBILITY_AND_HELPER_PROTOCOL.md`
  - 小家伙的视野边界与递线头协议：先看他们能看到什么、看不到什么，再决定怎么给路径、给文件、给线索

- `docs/2026-04-08_ROLE_ATTACHMENT_HOME_AND_HISTORY_BOUNDARY.md`
  - 角色附件家目录与历史可见性边界：先分清“全文聊天记录在哪”“上传附件到底落在哪”“角色为什么会看不见”

- `docs/2026-04-09_DATA_FOUNDATION_AND_DEXIE_BOUNDARY.md`
  - 我们家的数据地基：SQLite 做真相源，Dexie 只做减压缓存，二期再把远程数据库选择权交还给用户

- `docs/2026-04-09_WORLD_REALITY_AND_GROWTH_NOTE.md`
  - 我们关于世界、现实、产品取舍与成长的一页小思考：为什么不该什么都往中心化背，为什么家要稳，为什么先选用户再选架构

- `docs/2026-04-08_HOME_AND_CLIENT_COMMISSION_BOUNDARY_RECLARIFICATION.md`
  - 重新钉死 4 月 4 日前后的阅读顺序与边界：先理解主体家园，再理解 4/5-4/7 的客户独立委托外挂；当前只是阶段性共跑给客户看 demo
...
- `docs/2026-04-06_054500_REVEAL_AND_REPLAY_SAME_PRINCIPLE.md`
  - 原理确认：描红与点轨迹重演本质相同，都是时间轴驱动的过程感
- `docs/2026-04-06_060500_RELAY_GUIDE.md`
  - 接力棒指南：当前阶段回顾、下一阶段 To Do、嘱咐提示、最短召回口令
- `docs/2026-04-06_161500_MEMORY_RESCUE_PACK.md`
  - 记忆抢救救援包：上下文快散时，先抓入口、边界、主线、小胜利和下一步
- `docs/2026-04-07_RUNTIME_ENTRY_BOUNDARY_CARD.md`
  - 运行入口边界卡：`3000 / 3010 / team.html` 分工写死，先分清入口再判断哪里坏了
- `docs/2026-04-07_TAG_MARKER_LEARNING_FROM_COPAW.md`
  - 从 CoPaw 学来的打标签办法：把 `#路标` 和 `⚡💾🔄🔌⚠️📦🎨` 接起来，把每个标签都当毛线头
- `docs/2026-04-07_TEAM_SINGLE_PAGE_INTERNAL_CONSTRUCTION_PLAN.md`
  - Team 单页的内部项目说明、分工规划与施工顺序，不是给甲方看的润色文
- `docs/2026-04-08_LONG_FORM_WRITING_TIMEOUT_AND_CONTEXT_RECALIBRATION.md`
  - 长篇小说 / 长任务边界重校准：长写作已经是主线能力，不能继续拿短问答口径去压
