...
const ROLE_BADGE_CLASS: Partial<Record<string, string>> = {
  ...
  designer: 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300',
  analyst: 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300',
};
...
const CoworkSessionItem: React.FC<CoworkSessionItemProps> = ({
  ...
  const pinButtonLabel = session.pinned ? '取消置顶' : '置顶对话';
  const actionLabel = '更多操作';
  const renameLabel = '重命名';
  const deleteLabel = '删除对话';
  ...
  return (
    <div
      ...
      className={`group relative min-h-[60px] rounded-2xl border px-3 py-2.5 cursor-pointer transition-all duration-150 ${
        isActive
          ? 'border-white/65 bg-white/76 shadow-sm dark:border-white/10 dark:bg-white/[0.08]'
          : 'border-transparent bg-white/30 hover:border-white/45 hover:bg-white/52 dark:bg-white/[0.03] dark:hover:border-white/10 dark:hover:bg-white/[0.06]'
      }`}
    >
      {/* Content area */}
      <div className="flex items-start">
        ...
        <div className={`flex-1 min-w-0 ${!isBatchMode ? 'pr-12' : ''}`}>
          <div className={`flex items-center mb-1 ${showStatusIndicator ? 'gap-2' : 'gap-0'}`}>
            ...
            {showStatusIndicator && (
              <span
                ...
                title={showRunningIndicator ? (statusLabelMap[session.status] || session.status) : undefined}
              />
            )}
            {isRenaming ? (
              <input
          ...
          <div className="flex items-center gap-2 text-xs dark:text-claude-darkTextSecondary text-claude-textSecondary">
            ...
            {session.agentRoleKey && (
              <span className={`rounded-full px-1.5 py-0.5 text-[10px] whitespace-nowrap ${ROLE_BADGE_CLASS[session.agentRoleKey] ?? 'bg-slate-100 text-slate-700 dark:bg-slate-800/60 dark:text-slate-300'}`}>
                {AGENT_ROLE_SHORT_LABELS[session.agentRoleKey] ?? session.agentRoleKey}
              </span>
            )}
          </div>
        </div>
      ...
      <div
        className={`absolute right-1.5 top-1.5 transition-opacity ${
          isRenaming
            ...
            : forceShowActions || session.pinned
              ? 'opacity-100'
        ...
        <div className="flex items-center gap-1">
          {showQuickDelete && (
            <button
              ...
              className="p-1.5 rounded-lg bg-red-500/10 text-red-500 shadow-sm hover:bg-red-500/15 transition-colors"
          ...
          <button
            ref={actionButtonRef}
            onClick={openMenu}
            className="p-1.5 rounded-lg bg-white/75 text-claude-textSecondary shadow-sm hover:bg-white dark:bg-white/[0.08] dark:text-claude-darkTextSecondary dark:hover:bg-white/[0.12] transition-colors"
      ...
      {menuPosition && (
        <div
          ref={menuRef}
          className="fixed z-50 min-w-[180px] rounded-2xl border border-white/55 bg-white/92 shadow-xl backdrop-blur-xl overflow-hidden dark:border-white/10 dark:bg-claude-darkSurface/95"
          ...
          {menuItems.map((item) => (
            ...
            className={`w-full flex items-center gap-2 px-3 py-2 text-left text-sm transition-colors ${
                item.tone === 'danger'
                  ...
                  : 'dark:text-claude-darkText text-claude-text hover:bg-black/[0.04] dark:hover:bg-white/[0.06]'
              }`}
            >
