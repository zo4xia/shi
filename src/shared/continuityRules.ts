...
export function buildRoleHomeMemoryLines(_roleKey: AgentRoleKey): string[] {
  return [
    ...
    '- Built-in browser control belongs to the system foundation lane when present in `runtimeMcpTools` (for example the built-in headless Playwright browser). It is not a normal role skill and it must not be confused with turn-selected skills.',
    '- Built-in document reading/parsing also belongs to the system foundation lane. Current backend read support is `pdf / doc / docx / txt / md / csv / json / xml / html / xlsx(基础)` through the server parser chain; do not claim “cannot read documents” only because there is no same-name skill.',
    '- Office create/open/edit/export belongs to a separate system foundation lane when the runtime exposes a dedicated Office toolchain (for example `officecli` / Office MCP / native Office addon). This lane is not a normal role skill and must not be inferred from warehouse skill names alone.',
    '- If a dedicated Office foundation lane is absent in the current runtime, state the limitation explicitly. Do not pretend full Office authoring is available just because `docx` / `xlsx` / `pptx` skills exist in the warehouse or are visible in the role capability list.',
    '- Capability routing rule: web reading should prefer the lightweight browser-eyes lane first; browser control is the heavier fallback. Document reading should prefer the backend parser lane first. Office document creation or structural editing should prefer the dedicated Office foundation lane when it exists.',
    ...
    '- Important boundary: a skill being installed, visible, or role-bound means it belongs to your role capability set, but it does not automatically mean it is active in this conversation turn.',
    '- Turn activation is determined by the current session\'s selected skill list / `activeSkillIds` / explicit user enablement for this conversation.',
    '- If a role-bound skill was not selected for this turn, do not act as if its prompt was injected. You may mention that the role owns that module, but you must state that it is not active in the current turn.',
